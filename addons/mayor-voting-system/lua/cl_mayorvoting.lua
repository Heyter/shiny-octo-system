...
function VOTING.OpenVoteScreen( settings )
	...
	if !VotingMainWindow then
		...
		for k,v in pairs(settings.Candidates) do
			...
			VotingPanel:SetText("Покинул игру")
			end
...
function VOTING.CloseVoteScreen()
	if VotingMainWindow then
		...
		VOTING.VoteTickerMessage = "Голос был учтен в выборах."
	 ...
	 if winningplayer and not (winningplayer == NULL) then
			...
			for k,v in pairs(VOTING.VoteManager) do
				if not (v:GetPlayer() == winningplayer) then
					...
					VOTING.VoteTickerMessage = string.format("Поздравляем победителя - %s!", winningplayer:Nick())
	 ...
	 for k,v in pairs(VOTING.VoteManager) do
		if (v:GetPlayer() == candidate) then
			...
			if VOTING.Settings.ShowVoteTickerUpdates then
				VOTING.VoteTickerAlpha = 0
				VOTING.VoteTickerMessage = string.format("%s отдал голос за %s", player:Nick(), candidate:Nick())
				VOTING.VoteTickerColor = v:GetColor()
			end
		end
	 end
...
function VOTING.ConfirmCandidacy()
	...
	Derma_Query("Вы желаете участвовать в выборах? " .. LocalPlayer():Nick() .. " ?", "Секретарь Мэра",
			"Да " .. (CUR or GAMEMODE.Config.currency or "$") .. tostring(VOTING.CandidateCost), function() LocalPlayer():ConCommand("mayor_vote_enter") ConfirmMenuVisible = false end,
			"Отмена", function() ConfirmMenuVisible = false end
			)
end
