...
SWEP.menuButtons[3] = {}
SWEP.menuButtons[3].buttonText = "Арестовать";
SWEP.menuButtons[3].left = true;
SWEP.menuButtons[3].func = function(owner,ent)
	if owner:GetPos():Distance(ent:GetPos()) > 150 then return end;
	if not ent.stunnedBaton then DarkRP.notify(owner, 1, 4, "Сначала оглушите цель!") return end;
    ...
    if not jpc or jpc == 0 then
        ...
        if not ent.Babygod then
            ...
            if owner.SteamName then
                DarkRP.log(owner:Nick() .. " (" .. owner:SteamID() .. ") арестовал " .. ent:Nick(), Color(0, 255, 255))
            end
        else
...
SWEP.menuButtons[2] = {}
SWEP.menuButtons[2].buttonText = "В розыск";
...
SWEP.menuButtons[4] = {}
SWEP.menuButtons[4].buttonText = "Ордер на обыск";
