...
function PANEL:Frame()
	...
	self.StatusBar.Paint = function( pnl, w, h )
		...
		if tonumber(self.StatusBar.NewVersion) != 0.0 and tonumber(self.StatusBar.NewVersion) > tonumber(TCB_Settings.Version) then
			VersionText = "Версия: "..TCB_Settings.Version.." | Доступна Новая Версия! ("..self.StatusBar.NewVersion..")"
		else
			VersionText = "Версия: "..TCB_Settings.Version
		end

		draw.DrawText( VersionText, "TCB_F4Menu_Small", 10, 6, TCB_Settings.TextColor )

		draw.DrawText( "Код написал: TheCodingBeast", "TCB_F4Menu_Small", 886, 6, TCB_Settings.TextColor, TEXT_ALIGN_RIGHT )
			...
			CurButton.Paint = function( pnl, w, h )
				...
				if button['info'] then
					if button['func'] > 0 then
						...
						draw.RoundedBox( 4, w - 50 + 0, 8 + 0, 40 - 0, 26 - 0, TCB_Settings.SoftBlack )
						draw.RoundedBox( 4, w - 50 + 2, 8 + 2, 40 - 4, 26 - 4, TCB_Settings.ButtonColor3 )

					else

						draw.RoundedBox( 4, w - 50 + 0, 8 + 0, 40 - 0, 26 - 0, TCB_Settings.SoftBlack )
						draw.RoundedBox( 4, w - 50 + 2, 8 + 2, 40 - 4, 26 - 4, TCB_Settings.ButtonColor2 )

					end
					draw.DrawText( tonumber(button['func']), "TCB_F4Menu_Medium", w - 30, 11, TCB_Settings.WhiteColor, 1 )
