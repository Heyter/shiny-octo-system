...
if !ARCLib.Version || ARCLib.Version < ver then -- This checks if another ARCLib already exists. Probably used by another of my Addons.
	...
	function ARCLib.BetweenNumberScaleReverse(min,inp,max)
		assert(isnumber(min),"Bad argument #1. I wanted a number, but I got a fucking "..type(min))
		assert(isnumber(inp),"Bad argument #2. I wanted a number, but I got a fucking "..type(inp))
		assert(isnumber(max),"Bad argument #3. I wanted a number, but I got a fucking "..type(max))
		return (ARCLib.BetweenNumberScale(min,inp,max)-1)*-1
	end
	function ARCLib.BetweenNumberScale(min,inp,max)
		assert(isnumber(min),"Bad argument #1. I wanted a number, but I got a fucking "..type(min))
		assert(isnumber(inp),"Bad argument #2. I wanted a number, but I got a fucking "..type(inp))
		assert(isnumber(max),"Bad argument #3. I wanted a number, but I got a fucking "..type(max))
		if inp <= min then
	...
	function ARCLib.TouchingBox(b1x1,b1x2,b1y1,b1y2,b2x1,b2x2,b2y1,b2y2) -- Check if box 1 is inside box 2.
	...
	function ARCLib.ConvertColor(col)
		assert( col , "ARCLib.ConvertColor: I wanted a color, but I got absolutely nothing!" )
   ...
   function ARCLib.ShortScale(num,dec) -- Formats a number to a string using the short number scale
	...
	if SERVER then
		...
		function ARCLib.NotifyBroadcast(str,typ,time,sound)
			net.Start("ARCLib_Notify")
			net.WriteString(str)
			net.WriteUInt(typ,4)
			net.WriteUInt(time,16)
		...
		net.Receive( "ARCLib_FullyLoaded", function(length,ply)
			if IsValid(ply) && !table.HasValue(fullyloadedplayers,ply) then
				MsgN(ply:Nick().." is now fully loaded!")
				net.Start("ARCLib_FullyLoaded")
				net.WriteEntity(ply)
				net.Broadcast()
				hook.Call( "ARCLib_OnPlayerFullyLoaded" ,nil,ply)
				table.insert(fullyloadedplayers,ply)
			end
		end)
		hook.Add("PlayerDisconnected", "ARCLib_FullyLoadedConfirm", function(ply)
			if IsValid(ATMThing) then 
				table.RemoveByValue(fullyloadedplayers,ply) 
			end
		end)
		...
		function entity:ARCLib_SetAnimation(anim)
			self:ARCLib_SetAnimation(self:LookupSequence(anim))
		end
		...
		function ARCLib.CutOutText(text,font,length)
			if !isstring(text) then return type(text) end
			if ARCLib.CachedStringsCut[util.CRC(text..font..length)] then -- I wonder if this will actually save performence...
