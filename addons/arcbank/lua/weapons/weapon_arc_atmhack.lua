...
function SWEP:SecondaryAttack()
    ...
    self:SetNextSecondaryFire( CurTime() + 1.5 )
	if SERVER then
		self.Settings[3] = ARCBank.Settings["hack_max"]
		self.Settings[4] = ARCBank.Settings["atm_hack_charge_rate"]
...
function SWEP:Initialize()
	...
	if CLIENT then
		self.chargerate = 1.5
		self.hacktime = math.Round((((500/200)^2+28)*(1+booltonumber(false)*3)))
...
function SWEP:DrawHUD()
	local power = math.floor((0-self.chargerate)*(self.energystart - CurTime()))
	draw.SimpleText( "Hacking Power: "..arc_timesentence(power),"ARCBankCard", surface.ScreenWidth() - 48*2, surface.ScreenHeight() - 48 - 24, Color(255,255,0,255),TEXT_ALIGN_RIGHT , TEXT_ALIGN_TOP  ) 
	if power < (self.hacktime - self.hacktimeoff) then
		draw.SimpleText( "Not enough Energy","ARCBankCard", surface.ScreenWidth() - 48*2, surface.ScreenHeight() - 48, Color(255,0,0,255),TEXT_ALIGN_RIGHT , TEXT_ALIGN_TOP  )
	elseif power > (self.hacktime + self.hacktimeoff) then
		draw.SimpleText( "Optimal Energy","ARCBankCard", surface.ScreenWidth() - 48*2, surface.ScreenHeight() - 48, Color(0,255,0,255),TEXT_ALIGN_RIGHT , TEXT_ALIGN_TOP  )
	else
		draw.SimpleText( "Chance of success: "..tostring(math.floor(((power - (self.hacktime - self.hacktimeoff))/(2*self.hacktimeoff))*100)).."%","ARCBankCard", surface.ScreenWidth() - 48*2, surface.ScreenHeight() - 48, Color(255,255,0,255),TEXT_ALIGN_RIGHT , TEXT_ALIGN_TOP  )
	end
end
if CLIENT then
	net.Receive( "arcatmhack_gui", function(length)
		...
		weapon.chargerate = setting[4]
		weapon.hacktime = math.Round((((setting[1]/200)^2+28)*(1+booltonumber(setting[2])*3)))
		...
		HackTimeL:SetText( "Estimated Hack Time: "..arc_timesentence(weapon.hacktime).."\nGive or Take: "..arc_timesentence(weapon.hacktimeoff) )
		...
		StealthCheckbox:SetText( "Distributed Account Hack" )                   // Set the text next to the box
		...
		NumSlider2:SetMax(setting[3])
		NumSlider2:SetDecimals(0)
		NumSlider2:SetValue( setting[1] )
		NumSlider2.OnValueChanged = function( panel, value )
			if value%25 != 0 then
				NumSlider2:SetValue( math.Clamp(math.Round(value/25)*25,0,setting[3]) )
			end
			--math.Clamp(NumSlider2:GetValue(),0,setting[3])
			weapon.hacktime = math.Round((((math.Round(value/25)*25)/200)^2+28)*(1+booltonumber(StealthCheckbox:GetChecked())*3))
			weapon.hacktimeoff = math.Round(weapon.hacktime^0.725)
			HackTimeL:SetText( "Estimated Hack Time: "..arc_timesentence(weapon.hacktime).."\nGive or Take: "..arc_timesentence(weapon.hacktimeoff) )
			HackTimeL:SizeToContents()
		end
		StealthCheckbox.OnChange = function( panel, value )
			NumSlider2:SetValue( math.Clamp(math.Round(NumSlider2:GetValue()/25)*25,0,setting[3]) )
			weapon.hacktime = math.Round((((math.Round(NumSlider2:GetValue()/25)*25)/200)^2+28)*(1+booltonumber(value)*3))
			weapon.hacktimeoff = math.Round(weapon.hacktime^0.725)
			HackTimeL:SetText( "Estimated Hack Time: "..arc_timesentence(weapon.hacktime).."\nGive or Take: "..arc_timesentence(weapon.hacktimeoff) )
			HackTimeL:SizeToContents()
		end
		...
		OkButton.DoClick = function()
			...
			net.WriteTable({math.Clamp(NumSlider2:GetValue(),0,setting[3]),StealthCheckbox:GetChecked()})
			net.SendToServer()
		end
	end)
SWEP.VElements = {
...
SWEP.WElements = {
	...
	**************************/
	...
	net.Receive( "arcatmhack_gui", function(length)
		...
		if settings[1] < 0 || settings[1] > ARCBank.Settings["hack_max"] then
			ARCBank.FuckIdiotPlayer(ply,"INVALID_HACK_OPERATION")
		end
		settings[3] = ARCBank.Settings["hack_max"]
		if (!weapon.ARCBank_IsHacker) then return end
