...
function VOTING.SetupDarkRPTeam()
	...
	if VOTING.OnlyEnterUsingNPC then 
		VOTING.Team.CustomCheckFailMsg = "You must speak to the NPC to enter the election!"
		return false
	else
	...
	TEAM.CustomCheckFailMsg = "Please confirm your mayor candidacy!"
...
function VOTING.CastVote(ply,cmd,args)
	...
	if not VOTING.AllowCandidatesToVote and ply.HasEnteredVoting then 
		VOTING.SendDarkRPNotice(ply, 1, 4, "Sorry candidates cannot vote in the election.") 
		return 
	end
...
function VOTING.EnterMayorVote(ply, cmd, args)
	...
	if VOTING.NextElectionAllowed and not (CurTime() >= VOTING.NextElectionAllowed) then
		local timemsg = string.format("You need to wait another %i minutes to enter!", math.Round((VOTING.NextElectionAllowed - CurTime()) / 60 ))
	...
	if not VOTING.AllowNewElectionWithMayor then
		for k,v in pairs(player.GetAll()) do
			if (v:Team() == VOTING.TeamID) then
				VOTING.SendDarkRPNotice(ply, 1, 4, "There is already a mayor elected!")
				return false
			end
		end
	end
	...
	if VOTING.InProgress then
		VOTING.SendDarkRPNotice(ply, 1, 4, "There is already an election in progress!")
	return false end
	
	if ply.HasEnteredVoting then
		VOTING.SendDarkRPNotice(ply, 1, 4, "You have already entered this election.")
	return false end
	...
	if (#VOTING.CurrentCandidates) >= VOTING.MaximumCandidates then
		VOTING.SendDarkRPNotice(ply, 1, 4, "There is already " .. tostring(VOTING.MaximumCandidates) .. " , sorry." )
	return false end
	...
	if VOTING.CandidateCost and VOTING.CandidateCost > 0 then
		...
		VOTING.SendDarkRPNotice(ply, 1, 4, "You cannot afford the entry costs, sorry.")
		return false end
	...
	VOTING.SendDarkRPNotice(ply, 2, 4, "You have been entered into the mayor elections. Good luck!")
	...
	if (#VOTING.CurrentCandidates >= VOTING.MinimumCandidates) then
		VOTING.AboutToBegin = true
			VOTING.ChatNotice("The next mayor election will begin in " .. tostring(VOTING.AboutToBeginTime) .. " seconds!")
		...
		function()
			...
			VOTING.ChatNotice("The voting is about to begin. Good luck!")
...
if not VOTING.InProgress then return end
	...
	if (winningvotes == 0) then
		VOTING.ChatNotice("No votes were cast in the election.")
	...
	elseif IsValid(candidate) then
		...
		net.Send(player.GetAll())
		VOTING.ChatNotice(string.format("Congratulations to the winner, %s!", candidate:Nick()))
		--Update player team
		...
		VOTING.ChatNotice("The winning mayor candidate has left the server!")
		net.Start("Voting_EndVote")
		net.Send(player.GetAll())
	end
...
if IsValid(vic) and VOTING.TeamID == vic:Team() then
	if VOTING.DemoteMayorOnDeath then
		...
		VOTING.ChatNotice("The mayor has been killed and a new mayor must be elected!")
