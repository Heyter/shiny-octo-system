...
net.Receive("meetuppoint", function()
	...
	sendNotify( LocalPlayer(), meetupdata[3].. " "..ORGS_Lang.meetset.." "..meetupdata[4]..". Информация: " ..meetupdata[1], NOTIFY_HINT )
	meetup = meetupdata
...
function drawMeetupPoint()
	if meetup then
		...
		cam.Start3D2D( meetup[2] + Vector( 0,80, 80 + 10 * math.abs(math.sin(CurTime() * 0.8)) ) , Angle(0, ang.y, 90), 1 )
            draw.DrawText( ORGS_Config.addonName.. " Место Встречи!\nСоздано игроком: "..meetup[3].."\n"..meetup[1], "meetupfont", 0, 0, Color(255, 255, 255), TEXT_ALIGN_CENTER )
        cam.End3D2D()
	end
end
