...
net.Receive("orgmenu", function()
	...
	function addMember( name, rank, online, steamid, lastseen )
		...
		if table.HasValue(data[2], "e") or table.HasValue(data[2], "f") then
			...
			playerOptions.DoClick = function()
				...
				if table.HasValue(data[2], "e") then
					memberOptions:AddOption("Исключить участника", function() 
						Derma_Query("Вы уверены что хотите исключить ".. name .." из "..ORGS_Config.addonName.."?", "Внимание!",
		...
		if player.GetBySteamID(steamid) then
			...
			playerLastseen:SetText("Последний раз заходил: " ..lastseen)
	...
	if ORGS_Config.enableOrgBank then
		if table.HasValue(data[2], "a") or table.HasValue(data[2], "b") then
			...
			subbalanceLabel:SetText( "Баланс банка организации" )
			...
			subdepositLabel:SetText( "Вложить свои деньги в банк." )
			...
			depositBlock.DoClick = function()
				Derma_StringRequest( ORGS_Lang.bank, ORGS_Lang.bankhowmuchdeposit, "Введите количество..", function(t) RunConsoleCommand("org_deposit", t) end)
			end
			...
			subwithdrawLabel:SetText( "Взять деньги из банка." )
			...
			withdrawBlock.DoClick = function()
				Derma_StringRequest( ORGS_Lang.bank, ORGS_Lang.bankhowmuchwithdraw, "Введите количество..", function(t) RunConsoleCommand("org_withdraw", t) end)
			end
	...
	if table.HasValue(data[2], "c") or table.HasValue(data[2], "d") or table.HasValue(data[2], "e") or table.HasValue(data[2], "f") or table.HasValue(data[2], "g") or table.HasValue(data[2], "h")  then
		...
		if table.HasValue(data[2], "c") then
			...
			messageEntry:SetText("Хей, вы пригласили ".. data[1]["name"] .." by " .. LocalPlayer():Nick() .. ".\nХотели бы вы присоединиться к нам?")
...
net.Receive("orgsadminselect", function()
	...
	orgslist.OnRowRightClick = function ( btn, line )
		...
		orgsOptions:AddOption("Delete Organisation", function() 
			Derma_Query("Вы точно хотите удалить организацию?", "Внимание!",
				"Yes", function() RunConsoleCommand("org_delete", orgslist:GetLine(line):GetValue(1)) orgslist:RemoveLine(line) end,
				"No")
		end )
		orgsOptions:AddOption("Изменить организацию", function() 
...
net.Receive("requestorganswer", function()
	...
	bankbalancelabel:SetText("Баланс банка")
	...
	membersList.OnRowRightClick = function ( btn, line )
		...
		memberOptions:AddOption("Kick Member", function() 
			Derma_Query("Ты уверен что хочешь кикнуть ".. membersList:GetLine(line):GetValue(2) .." из "..ORGS_Config.addonName.."?", "Внимание!",
	...
	editorg:SetText("Изменение организации")
	...
	deleteorg:SetText("Удаление организации")
