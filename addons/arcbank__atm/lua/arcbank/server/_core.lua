...
hook.Add( "PhysgunPickup", "ARCBank NoPhys", function( ply, ent ) if ent.IsAFuckingATM && ent.ARCBank_MapEntity then return false end if ent:GetClass() == "sent_arc_atmhack" && ent:GetPos():DistToSqr( ply:GetPos() ) < 6500 then ent:TakeDamage( 100, ply, ply:GetActiveWeapon() ) end end)
--self:GetPos():DistToSqr( self.UsePlayer:GetPos() ) > 25000
...
hook.Add( "PlayerGetSalary", "ARCBank PaydayATM DRP2.4", function(ply, amount)
	if ARCBank.Settings["use_bank_for_payday"] && ARCBank.AtmFunc(0,ply,amount,"") == ARCBANK_ERROR_NONE then
		ply:SendLua("notification.AddLegacy(\"Your paycheck has been sent to your ARCBank Account.\", NOTIFY_HINT, 4)")
	end
end)
...
function ARCBank.FuckIdiotPlayer(ply,reason)
	...
	if ply.ARCBank_AFuckingIdiot then
		...
		game.ConsoleCommand( "banid "..tostring(ARCBank.Settings["autoban_time"]).." "..ply:SteamID().."\n")
		game.ConsoleCommand("writeid\n")
		ply:SendLua("Derma_Message( \"You will be autobanned for "..arc_timestring( ARCBank.Settings["autoban_time"]*60 )..".\", \"You're a failure at hacking\", \"Shit, Looks like I'm an idiot.\" )")
		timer.Simple(10,function()
			if !IsValid(ply) then return end
			ply:Kick("ARCBank Autobanned for "..arc_timestring( ARCBank.Settings["autoban_time"]*60 ).." - Tried to be a L33T H4X0R ["..tostring(reason).."]") 
		end)
	else
...
ARCBank.SettingsDesc["atm_language"] = "What language should the ATM display? Choose between the following: en - fr - ger - pt_br - sp - nl"
...
ARCBank.SettingsDesc["autoban_time"] = "The amount of time a player will be banned for if they try to use an exploit. (minutes)"
ARCBank.SettingsDesc["debt_limit"] = "The debt limit of an account."

ARCBank.SettingsDesc["use_bank_for_payday"] = "All payday money will be sent to their bank account."

ARCBank.SettingsDesc["interest_enable"] = "Enable or disable giving out bank interest."

ARCBank.SettingsDesc["interest_time"] = "The interval time of the giving of interest. (hours)"

ARCBank.SettingsDesc["hack_max"] = "The maximum amount of money a player can hack at once."
...
function ARCBank.SettingsReset() --DO NOT EDIT THIS!!!!
	...
	ARCBank.Settings["atm_hack_notify"] = {"TEAM_POLICE","TEAM_MAYOR"} --DO NOT EDIT THIS!!!!
...
function ARCBank.PlayerCanAfford(ply,amount)
	if string.lower(GAMEMODE.Name) == "darkrp" then
		if string.StartWith( GAMEMODE.Version, "2.6." ) then
			return ply:canAfford(amount)
		else
			return ply:CanAfford(amount)
		end
	else
		ply:SendLua("notification.AddLegacy( \"This is an unsupported gamemode.\", 0, 5 )")
		return false
	end
end
...
function ARCBank._AtmFunc(take,ply,amount,groupname)
	if !ARCBank.Loaded then return ARCBANK_ERROR_NOT_LOADED end
	if amount < 0 then
		ARCBank.FuckIdiotPlayer(ply,"NEGATIVE_WITHDRAW_OR_DEPOSIT_OPERATION")
		return 15
	end
...
function ARCBank.Transfer(fromply,toply,fromname,toname,amount,reason)
	if !ARCBank.Loaded then return ARCBANK_ERROR_NOT_LOADED end
	if amount < 0 then
		ARCBank.FuckIdiotPlayer(fromply,"NEGATIVE_TRANSFER_OPERATION")
		return 15
	end
...
--[[
	...
	ARCBankMsg("Giving out interest every "..arc_timestring(ARCBank.Settings["interest_time"]*3600))
	timer.Create( "ARCBANK_INTEREST", ARCBank.Settings["interest_time"]*3600, 0, function() ARCBank.AddAccountInterest() end )
