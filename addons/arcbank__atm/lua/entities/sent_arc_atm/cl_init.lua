...
local asdqwefwqaf = surface.GetTextureID( "arc/atm_base/screen/welcome_animated_free" ) 
...
hook.Add( "CalcView", "ARCBank ATMCalcView",function( ply, pos, angles, fov )
...
function ENT:ShowNotify(msg,err,time)
	...
	if err > 0 then
		...
		timer.Simple(0.1,function()arcbank_plysound("arcbank/atm/beep.wav",self)end)
		timer.Simple(1.1,function()arcbank_plysound("arcbank/atm/beep.wav",self)end)
		timer.Simple(2.1,function()arcbank_plysound("arcbank/atm/beep.wav",self)end)
	end
...
function ENT:PushNumber(num)
	...
	arcbank_plysound("arcbank/atm/beep_short.wav",self)
	if num == 11 then
				...
				if self.UpgradingGroup then
					...
					if self.SearchingGroup then
						...
						if !self.GivingMoney then
							if !self.ScreenOptions[butt] || self.ScreenOptions[butt] == "" then return end
							--MsgN("hi!")
							self.NameTable = {}
