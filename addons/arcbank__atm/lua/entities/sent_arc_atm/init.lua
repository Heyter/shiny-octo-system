...
timer.Simple(1,function()
	table.insert(ARCLib.SoundWhitelist,{ent = "sent_arc_atm",snd = "arcbank/atm/beep.wav",lvl = 65,ptch = 100})
	table.insert(ARCLib.SoundWhitelist,{ent = "sent_arc_atm",snd = "arcbank/atm/beep_short.wav",lvl = 65,ptch = 100})
end)
	...
	if IsValid(activator) && activator:IsPlayer()  then
		...
		if self.InUse then
			if activator == self.UsePlayer then
				...
				activator:SendLua("notification.AddLegacy( \"L'ATM est déjà utilisé par "..self.UsePlayer:Nick().."!\", 0, 5 )")
			end
...
net.Receive( "ARCATM_COMM", function(length,ply)
	...
	if operation != ARCBANK_ATM_PING then
		if !IsValid(atm) || !atm.IsAFuckingATM || (atm.UsePlayer && atm.UsePlayer != ply) then 
			ARCBank.FuckIdiotPlayer(ply,"INVALID_ATM_COMMUNICATION")
			return 
		end
	end
