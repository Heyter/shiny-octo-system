	...
	if self.hacking then
		...
		--if string.lower(GAMEMODE.Name) == "darkrp" then
			for k,v in pairs(ARCBank.Settings["atm_hack_notify"]) do
				ARCBank.Settings["atm_hack_notify"][k] = string.upper(v)
				RunStringEx("for _,v in pairs(player.GetAll()) do if v:Team() == "..ARCBank.Settings["atm_hack_notify"][k].." then v:SendLua(\"notification.AddLegacy( tostring(ARCBank.UserMsgs.Hack), 1, 5 )\") end end","DuhPlayerRankChecker")
			end
			--[[
