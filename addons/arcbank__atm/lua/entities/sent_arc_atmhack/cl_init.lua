...
net.Receive( "ARCATMHACK_BEGIN", function(length)
	...
	if !IsValid(atm) || !IsValid(hacker) then return end
	hacker.hacking = hack
	hacker.ori = orient
	if hack then
...
function ENT:Draw()
	...
	cam.Start3D2D(DisplayPos, self.displayangle1, 0.0245)
		...
		if IsValid(atm) then
			...
			draw.SimpleText( "Progress: "..tostring(math.Clamp(math.Round(atm.WaitPercent*100),0,100)).."%", "ARCBankATM", -49, 6, Color(0,0,0,255), TEXT_ALIGN_LEFT , TEXT_ALIGN_CENTER )
			surface.DrawRect(-48, 24, math.Clamp(atm.WaitPercent*96,0,96), 10 ) 
			surface.DrawRect(-48, 12, math.Clamp((atm.WaitPercent^2)*96,0,96), 10 )
