...
if SERVER then
	...
	function _R.Player.ChatPrint(self, text)
		--print(text)
		umsg.Start("JChat.SendChatPrint")
		umsg.Bool(true)
		umsg.End()
		oldchatprint(self, text)
	end	
	local oldprintmessage = _R.Player.PrintMessage
	function _R.Player.PrintMessage(self, enum, text)
...
JChat.FadeTime = CreateClientConVar("jchat_fadetime", 0.35):GetInt()
